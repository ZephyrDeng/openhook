<script>
  import { filters } from '../stores/api.js'
  import { toast } from '../stores/toast.js'
  import Modal from '../components/Modal.svelte'
  import { Plus, Filter, Pencil, Trash2, Save, X } from 'lucide-svelte'
  ...
  async function save() {
    try {
      ...
      if (editingId === 'new') {
        await filters.create(body)
        toast.success('已创建')
      } else {
        await filters.update(editingId, body)
        toast.success('已更新')
      }
      editingId = null
      load()
  ...
  async function doDelete() {
    try {
      await filters.delete(deletingItem.id)
      toast.success('已删除')
      load()
    } catch (e) {
      toast.error('删除失败')
...
<div class="flex flex-col h-full">
  <div class="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border-subtle)]">
    <div>
      <h1 class="text-xl font-semibold text-[var(--color-text-primary)]">过滤器</h1>
      <p class="text-sm text-[var(--color-text-secondary)] mt-0.5">配置消息过滤规则</p>
    </div>
  ...
  <div class="flex-1 overflow-auto p-6">
    ...
    {:else if items.length === 0 && !editingId}
      <div class="flex flex-col items-center justify-center h-64 text-center">
        <div class="w-12 h-12 rounded-full bg-[var(--color-bg-tertiary)] flex items-center justify-center mb-4"><Filter size={24} class="text-[var(--color-text-tertiary)]" /></div>
        <p class="text-sm font-medium text-[var(--color-text-primary)]">还没有过滤器</p>
        ...
        {#if editingId}
          <div class="card space-y-4">
            <div class="flex items-center justify-between">
              <h3 class="text-sm font-medium">{editingId === 'new' ? '新建过滤器' : '编辑过滤器'}</h3>
            ...
            <div class="flex items-center gap-2">
              <input type="checkbox" id="f-status" bind:checked={form.status} class="w-4 h-4 accent-[var(--color-accent)]" />
              <label for="f-status" class="text-sm">启用</label>
            </div>
...
<Modal show={showDeleteModal} title="确认删除" onclose={() => showDeleteModal = false}>
  <div class="text-sm text-[var(--color-text-secondary)]">确定要删除过滤器 <strong class="text-[var(--color-text-primary)]">{deletingItem?.name}</strong> 吗？</div>
