...
if ! OPENHOOK_BOOTSTRAP_DRY_RUN=1 OPENHOOK_DOMAIN=commute-planner.site "${SCRIPT}" >/tmp/openhook-bootstrap.out 2>/tmp/openhook-bootstrap.err; then
...
grep -q "commute-planner.site" /tmp/openhook-bootstrap.out
